...
class QueryRouter:
    def __init__(self, llm):
        ...
        self.router_prompt = PromptTemplate(
            ...
            2. RAG - Use this for questions about specific incidents, detailed descriptions, or narrative information. Note that terms like "smoked sausage" and "thermal processed pork meat" are included under the product category in our dataset but not limited to these two examples. There are many  
    ...
    def _initialize_sql_chain(self):
        ...
        try:
            db = SQLDatabase.from_uri(
                f"mysql+pymysql://{os.getenv('DB_USER')}:{os.getenv('DB_PASSWORD')}@{os.getenv('DB_HOST')}:3305/{os.getenv('DB_NAME')}",
...
def main():
    st.title("Food Safety Incidents Analysis")
